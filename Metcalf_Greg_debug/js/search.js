...
(function(){
	...
	var validqte = function(query){ //Added opening brace
		
		// Trim whitespace from start and end of search query
		while(query.charAt(0) == " "){ //Added == to check equality to whitespace
			query = query.substring(1, query.length);
		};
		while(query.charAt(query.length-1) === " "){  //correctly spelled charAt, added whitespace between quotes
			query = query.substring(0, query.length-1);
		}; // Added closing brace
		
		// Check search length, must have 3 characters
		if(query.length < 3){
			alert("Your search query is too small, try again."); // Added end quote
	...
	var search = function(query){ //Added opening quote
		
		// SPLIT the user's search query string into an array
		var queryArray = query.split(" "); //Changed property from join to split
		...
		for(var i=0, j=db.length; i<j; i++){
			...
			var dbitem = db[i].toLowerCase().substring(0, dbTitleEnd); // Correctly spelled toLowerCase
			
			// loop through the user's search query words
			// save a lowercase variable of the search keyword
			for(var ii=0, jj=queryArray.length; ii<jj; ii++){
				var qitem = queryArray[ii].toLowerCase(); //Correctly spelled toLowerCase
		...
		if(results.length == 0){ //Added == for equality check
			noMatch();
		}else{
			showMatches(results);
		};
	...
	var showMatches = function(results){
		...
		for(var i=0, j=results.length; i<j; i++){
			...
			title = results[i].substring(0, titleEnd); // Correctly spelled substring
